...
st.subheader("🕉️ BRAHMA AI : Studio (Audio & Poster Engine)")
...
if "diag_logs" not in st.session_state:
    st.session_state.diag_logs = [
        {"time": datetime.now().strftime("%H:%M:%S"), "msg": "System Ready. AI Poster & Audio Engine Active.", "color": "#38bdf8"}
    ]
...
# ==========================================
# 3. AI స్పీచ్ స్టైల్ & పోస్టర్ థీమ్ సెట్టింగ్స్
# ==========================================
with st.expander("⚙️ AI CONTROLS (స్పీచ్ స్టైల్ & పోస్టర్ థీమ్స్)", expanded=False):
    col_style, col_pause, col_poster_th = st.columns(3)
    with col_style:
    ...
    with col_poster_th:
        poster_theme = st.selectbox("🎨 పోస్టర్ థీమ్:", options=["ఆధ్యాత్మికం (Golden Divine)", "రక్తదానం / సేవా కార్యక్రమం (Red & White)", "ప్రకృతి / పచ్చదనం (Nature Green)", "రాయల్ బ్లూ (Corporate & Formal)"])
...
# ==========================================
# 7. ఏడు ప్రధాన యాక్షన్ కంట్రోల్స్ (పోస్టర్‌తో సహా)
# ==========================================
...
with b2:
    if active_text:
        if st.button("🖼️ AI POSTER", use_container_width=True):
            with st.spinner("Groq AI ద్వారా పోస్టర్ డిజైన్ అవుతోంది..."):
                poster_html = generate_ai_poster_html(active_text, theme=poster_theme)
                st.session_state.poster_html_data = poster_html
                add_log("AI పోస్టర్ సిద్ధమైంది!", "#4ade80")
